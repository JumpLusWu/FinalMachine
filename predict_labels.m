...
RF_models = cell(1, 9);
y_hat6 = NaN(size(test_inputs, 1), 9);
y_pre6 = zeros(size(train_inputs, 1), 9);
for j = 1:p
        % Using 500 trees
        RF_models{1, j} = TreeBagger(500, train_inputs(:,1:21),...
            Y_train(:, j), 'Method','regression');
        % Make prediction
%         y_pre3(:, j) = predict(RF_models{1, j}, tweet_reduced_X); 
        y_hat6(:, j) = predict(RF_models{1, j}, test_inputs(:,1:21));     
end

pred_labels = 0.9*y_hat1 + 1.2*y_hat3 + 1.1*y_hat4 + 0.8*y_hat5+y_hat6;
pred_labels = pred_labels ./5;
